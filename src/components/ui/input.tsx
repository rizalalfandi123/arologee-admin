...
const Input = React.forwardRef<HTMLInputElement, InputProps>(({ className, suffixIcon, type, prefixIcon, ...inputProps }, ref) => {
   return (
      <div className="relative">
         ...
         {suffixIcon && <Slot className="absolute top-[8px] right-3 w-5 h-5 text-placeholder">{suffixIcon}</Slot>}
      </div>
   );
});
