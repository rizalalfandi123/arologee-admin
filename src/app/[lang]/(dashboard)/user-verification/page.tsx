...
import SummaryCard, { type SummaryCardProps as SummaryCardItem } from "./summary-card";
...
const getSummaryValues: () => Promise<TotalRequestVerifiedResponse["data"]> = async () => {
   const { data } = await fetchApi<TotalRequestVerifiedResponse>("/user/total-request-verified", { method: "GET" });
...
export default async function UserVerification(props: UserVerificationProps) {
   ...
   return (
      <div className="flex flex-col gap-4 px-2">
         <h1 className="text-2xl p-2 font-semibold mb-2 md:p-6">{userVerificationDictionaries["user-overview"]}</h1>
